...
public class PlayerMotor2D : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (dashing.isDashing)
        {
            ...
            if (!dashing.forceEnd)
            {
                GetComponent<Rigidbody2D>().velocity = dashing.dashDir;
            }
        ...
        else
        {
            ...
            if (stuckTo != Surface.None && GetComponent<Rigidbody2D>().velocity.y < 0)
            {
            ...
            if (Time.time >= ignoreMovementUntil)
            {
                if (stuckTo == Surface.Ground)
                {
                    ...
                    if (accelerate)
                    {
                        GetComponent<Rigidbody2D>().velocity += movementDir * groundAcceleration * Time.fixedDeltaTime;
                    }
                    else
                    {
                        GetComponent<Rigidbody2D>().velocity = movementDir * maxGroundSpeed;
                    }
                }
                else
                {
                    ...
                    else
                    {
                            ...
                            stuckTo == Surface.None)
                        {
                            if (accelerate)
                            {
                                GetComponent<Rigidbody2D>().velocity += movementDir * airAcceleration * Time.fixedDeltaTime;
                            }
                            else
                            {
                                ...
                                GetComponent<Rigidbody2D>().velocity = new Vector2(
                                    ...
                                    GetComponent<Rigidbody2D>().velocity.y + movementDir.y * maxAirSpeed);
                            }
                        }
                    }
                }
            ...
            if (allowCornerGrab)
            {
                if (GetComponent<Rigidbody2D>().velocity.y < 0 || wallInfo.onCorner)
                {
                        ...
                        CheckIfAtCorner())
                    {
                        ...
                        if (wallInfo.onCorner)
                        {
                            ...
                            v.y = -1 * GetComponent<Rigidbody2D>().gravityScale * Physics2D.gravity.y * Time.fixedDeltaTime;
            ...
            if (allowWallCling)
            {
                if (GetComponent<Rigidbody2D>().velocity.y < 0 || wallInfo.clinging)
                {
                    if (stuckTo == Surface.LeftWall && movementDir.x < -heavyInputThreshold ||
                    stuckTo == Surface.RightWall && movementDir.x > heavyInputThreshold)
                    {
            ...
            if (allowWallSlide)
            {
                if (GetComponent<Rigidbody2D>().velocity.y < 0 || wallInfo.sliding)
                {
                        ...
                        stuckTo == Surface.RightWall && movementDir.x > heavyInputThreshold)
                    {
                        ...
                        // Set the y to one acceleration tick upwards against gravity.
                        v.y = -1 * GetComponent<Rigidbody2D>().gravityScale * Physics2D.gravity.y * Time.fixedDeltaTime - wallSlideSpeed;
                        GetComponent<Rigidbody2D>().velocity = v;
            ...
            if (jumping.isJumping && jumping.held)
            {
                if (Time.time < jumping.time + extraJumpHeight / CalculateJumpSpeed())
                {
                    ...
                    //       feel.
                    GetComponent<Rigidbody2D>().velocity += new Vector2(0, -1 * GetComponent<Rigidbody2D>().gravityScale * Physics2D.gravity.y * Time.fixedDeltaTime);
                }
            }
            ...
            if (jumping.pressed)
            {
                ...
                {
                    // Normal jump.
                    GetComponent<Rigidbody2D>().velocity = new Vector2(GetComponent<Rigidbody2D>().velocity.x, CalculateJumpSpeed() + jumping.extraSpeed);
                }
                ...
                else if (allowWallJump && stuckTo == Surface.LeftWall)
                {
                    ...
                    GetComponent<Rigidbody2D>().velocity = upRight * CalculateJumpSpeed() * wallJumpMultiplier;
                ...
                {
                    GetComponent<Rigidbody2D>().velocity = upLeft * CalculateJumpSpeed() * wallJumpMultiplier;
                    ignoreMovementUntil = Time.time + IGNORE_INPUT_TIME;
                    jumping.doubleJumped = false;
                }
    ...
    {
        return Mathf.Sqrt(-2 * baseJump * GetComponent<Rigidbody2D>().gravityScale * Physics2D.gravity.y);
    }
        ...
        set
        {
            if (_frozen != value)
            {
                ...
                if (_frozen)
                {
                    ...
                    frozenVelocity = GetComponent<Rigidbody2D>().velocity;
                    frozenGravity = GetComponent<Rigidbody2D>().gravityScale;
                ...
                else
                {
                    ...
                    GetComponent<Rigidbody2D>().velocity = frozenVelocity;
                    GetComponent<Rigidbody2D>().gravityScale = frozenGravity;
                }
            }
        }
    }
